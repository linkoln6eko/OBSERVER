...
static uint64_t g_last_bp_key_seq = 0;
...
/* Print BP key when it changes */
{
  uint64_t seq = bp_get_key_seq();
  if (seq != g_last_bp_key_seq) {
    g_last_bp_key_seq = seq;
    char hex[65];
    int n = bp_get_key_hex(hex, sizeof(hex));
    if (n > 0) {
      printf("[BP] key=%s (len=%d)\n", hex, n);
    } else {
      printf("[BP] key=<none>\n");
    }
  }
