...
processDMRvoice (dsd_opts * opts, dsd_state * state)
{
  ...
  mutecurrentslot = 0;
          bp_reset_superframe_for_slot(state->currentslot);
  msMode = 0;
  ...
  for (j = 0; j < 6; j++)
    {
      ...
      if (mutecurrentslot == 0)
        {
          if (state->firstframe == 1)
            ...
            {
              bp_on_air_xor(ambe_fr, state->currentslot);
              processMbeFrame (opts, state, NULL, ambe_fr, NULL);
              bp_on_air_xor(ambe_fr2, state->currentslot);
      ...
      if (mutecurrentslot == 0)
        {
          bp_on_air_xor(ambe_fr3, state->currentslot);
